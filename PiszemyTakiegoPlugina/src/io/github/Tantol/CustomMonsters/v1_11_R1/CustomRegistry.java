package io.github.Tantol.CustomMonsters.v1_11_R1;
...
import io.github.Tantol.CustomMonsters.CustomRegistryEntry;
...
final public class CustomRegistry {
	...
	public static void register(final CustomRegistryEntry cre) {
		...
		if (Dungeon.enabled) {
			...
			for (World w : Bukkit.getWorlds()) {
				final File wFile = new File(Dungeon.saveDirectory, w.getName());
				if (wFile.exists()) {
	...
	protected static void saveEntity(EntityInsentient e) {
		try {
			...
			final File cDir = new File(Dungeon.saveDirectory+System.getProperty("file.separator")+c.getWorld().getName()+System.getProperty("file.separator")+c.getX()+System.getProperty("file.separator")+c.getZ());
			cDir.mkdirs();
	...
	protected static boolean loadEntity(NBTTagCompound nbt, World w) {
		...
		try {
			...
			EntityInsentient e = clazz.getConstructor(World.class).newInstance(nmsW);
